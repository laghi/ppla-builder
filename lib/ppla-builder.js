...
module.exports = function() {
    ...
    PplaBuilder = {
        ...
        FONT_TYPE: {
            ...
            ':': { value: ':', subTypes:  [
                                            this.COURIER_SUBTYPES['Roman-8'],
                                            this.COUNTER_STYLE_RULE['ECMA-94'],
                                            this.COUNTER_STYLE_RULE['PCset'],
                                            this.COUNTER_STYLE_RULE['PCsetA'],
                                            this.COUNTER_STYLE_RULE['PCsetB'],
                                            this.COUNTER_STYLE_RULE['Legal'],
                                            this.COUNTER_STYLE_RULE['Greek'],
                                            this.COUNTER_STYLE_RULE['Russian']
                                          ]
                 }
        },
        defaults: {
        ...
        direction: function (newDirection) {
            if (_.contains(this.validDirections, newDirection))
                this.currentLine.direction = newDirection
            return this
        },
        fontType: function (newFontType, newFontSubType) {
            ...
            if (!newFontData)
                throw Error('The specified font type does not match any known font type')
            this.currentLine.fontType = newFontValue
            return this.fontSubType(newFontSubType)
        },
        fontSubType: function (newFontSubType) {
            var fontData = this.FONT_TYPE[this.currentLine.fontType]
            if (!fontData || !newFontSubType){
                console.log('Font subtype not defined')
                return this
            }
            if (!(_.contains(fontData.subTypes, newFontSubType )))
                throw Error('The specified font subtype cannot be used with the specified font type')
        ...
        x: function (xCoordinate) {
            var newXCoordinate = xCoordinate ? String(xCoordinate) : undefined
            if(newXCoordinate)
                this.currentLine.x = newXCoordinate
            return this
        },
        y: function (yCoordinate) {
            var newYCoordinate = yCoordinate ? String(yCoordinate) : undefined
            if(newYCoordinate)
